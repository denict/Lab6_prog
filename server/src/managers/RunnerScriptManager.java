...
public class RunnerScriptManager implements InputHandler {
    ...
    public String readLine() {
        try {
            BufferedReader reader = readers.getLast();
            String line = reader.readLine(); // null, если конец файла
            if (line == null) {
                return null;
            }
            return line;
    ...
    public boolean hasNextLine() {
        try {
            ...
            BufferedReader reader = readers.getLast();
            // Проверяем, есть ли еще строки в текущем файле
            if (reader.ready()) {
                return true;
            }
            // Если нет, проверяем, есть ли еще файлы в очереди
            if (readers.size() > 1) {
                readers.removeLast();
                return hasNextLine();
            }
            return false;
//            return readers.getLast().ready();
        } catch (IOException e) {
            return false;
        }
    }
