...
public class ExecuteScript extends Command implements Serializable {
    ...
    public Response execute(Request request) {
        ...
        } else if (request.getArgs() instanceof String[]) {
             pathName = ((String[]) request.getArgs())[0];
        }
...
//                        sb.append(commandManager.getCommands().get(nameCommand).execute(new Request(new ExecuteScript(commandManager, consoleInput, consoleOutput, new RunnerScriptManager()), args[0])).getMessage() + "\n\n");
                        ...
                        sb.append(commandManager.getCommands().get(nameCommand).execute(new Request(organization,commandManager.getCommands().get(nameCommand))).getMessage() + "\n\n");
                    } else if (nameCommand.equals("update_by_id")) { // объект коллекции + аргумент
                        Organization organization = new OrganizationBuilder(consoleInput, consoleOutput).build();
                        sb.append(commandManager.getCommands().get(nameCommand).execute(new Request(organization, commandManager.getCommands().get(nameCommand), args)).getMessage() + "\n\n");
                        ...
                        sb.append(commandManager.getCommands().get(nameCommand).execute(new Request(commandManager.getCommands().get(nameCommand), args)).getMessage() + "\n\n");
                    } else { // без аргументов
                        sb.append(commandManager.getCommands().get(nameCommand).execute(new Request(commandManager.getCommands().get(nameCommand))).getMessage() + "\n\n");}
